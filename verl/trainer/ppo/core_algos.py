...
def compute_grpo_outcome_advantage(
    ...
    with torch.no_grad():
        ...
        # # ====== 新增：Group-wise Softmax（最小侵入式） ======
        # if use_softmax:
        #     softmaxed_scores = torch.zeros_like(scores)
        #     for idx in id2score:
        #         mask = torch.tensor(index == idx, device=scores.device)  # [bs], bool
        #         group_scores = scores[mask] / temperature # [group_size]
        #         # Numerical stability
        #         group_scores = group_scores - group_scores.max()
        #         exp_vals = torch.exp(group_scores)
        #         probs = exp_vals / (exp_vals.sum() + epsilon)
        #         softmaxed_scores[mask] = probs
        #     scores = softmaxed_scores
        # # ===================================================

        # # ===================================================
        # if use_softmax:
        #     # Ensure it's float and binary
        #     scores = scores.float()
        #     # Step 2: Apply label smoothing: 1 → 1−δ, 0 → δ
        #     smoothed_scores = torch.where(scores == 1.0, 1.0 - 0.1, 0.1)  # (bs,)
...
def compute_policy_loss_vanilla(
    ...
    # # ===== Small-loss selection (minimal invasive) =====
    # # if getattr(config, "baseline", None) == "small_loss_select":
    # select_ratio = 0.5  # 固定 50%，不额外引入 config 字段

    # valid_mask = response_mask.bool()
    # valid_losses = pg_losses[valid_mask]

    # if valid_losses.numel() > 0:
    #     k = max(int(select_ratio * valid_losses.numel()), 1)
    #     threshold = torch.kthvalue(valid_losses.detach(), k).values

    #     small_loss_mask = (pg_losses <= threshold) & valid_mask
    #     response_mask = small_loss_mask.float()
    # # ===================================================

    # # ===== Sequence-level Small-loss selection =====
    # # if getattr(config, "baseline", None) == "small_loss_select":
    # select_ratio = 0.5  # 固定比例

    # valid_mask = response_mask.bool()  # [B, T]

    # # 每个 sequence 的 mean loss（只对有效 token 求均值）
    # token_count = valid_mask.sum(dim=1).clamp(min=1)  # [B]
    # seq_loss = (pg_losses * valid_mask).sum(dim=1) / token_count  # [B]

    # B = seq_loss.size(0)
    # k = max(int(select_ratio * B), 1)

    # # 选小 loss 序列（detach 防止排序进图）
    # threshold = torch.kthvalue(seq_loss.detach(), k).values
    # keep_seq = seq_loss <= threshold  # [B]

    # # 扩展为 token mask
    # keep_seq_mask = keep_seq.unsqueeze(1)  # [B,1]
    # response_mask = (valid_mask & keep_seq_mask).float()
    # # =================================================

    # # ===== Sequence-level Random selection =====
    # # if getattr(config, "baseline", None) == "random_select":
    # select_ratio = 0.5  # 固定 50%

    # valid_mask = response_mask.bool()  # [B, T]
    # B = valid_mask.size(0)

    # # 每个 sequence 随机采样
    # keep_seq = torch.rand(B, device=pg_losses.device) < select_ratio  # [B]

    # # 至少保留 1 个，避免全 0 batch
    # if not keep_seq.any():
    #     rand_idx = torch.randint(0, B, (1,), device=pg_losses.device)
    #     keep_seq[rand_idx] = True
