...
# def main(input_file, output_file, model_path, debug=False, remove_system=True, template='own', temperature=0.6, top_p=1.0, max_tokens=16384, n=1, force_generate=True, add_think_before_answer=False, add_oat_evaluate=False, any_true=False, skip_scoring=False, output_eval=None, no_split_think=False):
...
def generate_vllm(messages, model_path, template='own', temperature=0.6, top_p=1.0, max_tokens=4096, n=1):
    ...
    for i in range(len(messages)):
        ...
        if template == 'own': 
            ...
            gen_prompt = apply_llama_math_template(cur_message[0]['content'])
        elif template == 'prime':
