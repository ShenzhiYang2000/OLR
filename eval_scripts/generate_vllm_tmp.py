...
def labeling_responses(responses: list[str], golden_answer: str, return_predict_answers=False):
...
def apply_qwen3_math_template(question: str):
    return (
        "<|im_start|>system\nLet's think step by step and output the final answer within \\boxed{}.<|im_end|>\n<|im_start|>user\n"
        + question
        + "<|im_end|>\n<|im_start|>assistant\n"
...
# def main(input_file, output_file, model_path, debug=False, remove_system=True, template='own', temperature=0.6, top_p=1.0, max_tokens=4096, n=1, force_generate=True, add_think_before_answer=False, add_oat_evaluate=False, any_true=False, skip_scoring=False, output_eval=None, no_split_think=False):
def main(input_file, output_file, model_path, debug=False, remove_system=True, template='own', temperature=0.6, top_p=1.0, max_tokens=16384, n=8, force_generate=True, add_think_before_answer=False, add_oat_evaluate=False, any_true=False, skip_scoring=False, output_eval=None, no_split_think=False):
    ...
    for i in tqdm(range(len(outputs)), total=len(outputs)):
        ...
        # if labels is None:
        #     try:
        #         labels, predicted_answers = labeling_responses([generated_text,], answer, return_predict_answers=True)
...
def generate_vllm(messages, model_path, template='own', temperature=0.6, top_p=1.0, max_tokens=4096, n=1):
    #vllm模型加载
    ...
    for i in range(len(messages)):
        ...
        if template == 'own': 
            ...
            # gen_prompt = apply_qwen3_math_template(cur_message[0]['content'])
        elif template == 'prime':
